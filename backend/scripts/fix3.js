...
// Reverse mapping for Windows-1252 specific characters back to their byte value
const win1252Rev = {
...
function decodeMojibake(moji) {
    ...
    for (let i = 0; i < moji.length; i++) {
        ...
        if (win1252Rev.hasOwnProperty(char)) {
            ...
            // It should be 0xA0 to 0xFF or 0x00 to 0x7F
...
// Regex to find sequences of Windows-1252 mapped characters starting with 'ð' (\u00F0) or 'â' (\u00E2) or 'Â' (\u00C2) or 'Î' (\u00CE)
// And containing at least 2 characters from the set
...
for (const file of files) {
  ...
  content = content.replace(mojibakeRegex, (match) => {
      ...
      // If the decoded string doesn't look completely wrong (e.g., doesn't contain replacement char \uFFFD)
      if (!decoded.includes('\uFFFD')) {
          return decoded;
      }
      return match;
